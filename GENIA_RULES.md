...
- E13-6 proves the existing Outcome-aware validated-pipeline composition, E13-7 changes no semantics while verifying runnable release examples and synchronizing implemented truth, and E13-8 changes no semantics while completing audit/distillation. R13 is release-complete and remains Experimental.
...
## 27) `execution.process` direct-execution invariants

- `execution.process(capability, request)` is capability-mediated: `capability` is an opaque, host-created value with no Genia-callable constructor, no ambient/global instance, and no `host.supports(...)` operation. `request.executable` is a provider-bound symbolic identity resolved through that capability's own private bindings, never a portable `PATH` search and never an OS path/command string supplied directly by Genia source.
- Direct execution is structured-argv only, never shell-string execution: each `request.args` element becomes exactly one child argv element after `executable`, with no quoting, splitting, glob expansion, variable interpolation, or command substitution performed by any layer. This is a permanently separate surface from the Python-host-only shell pipeline stage (`$(...)`, section 3 above): neither wraps or is implemented in terms of the other.
- A completed child's nonzero exit status is ordinary result data (`some({exit_code, stdout, stderr})`), never a failure Outcome; only an inability to complete the attempt at all is `err(...)`. `stdout`/`stderr` are opaque bytes, independently bounded at `1,048,576` bytes each, never implicitly decoded or truncated without an error.
- A request value that recursively contains a protected leaf is rejected (reusing the existing R10 `contains_protected`/`reject_protected` machinery) before any resolution or provider effect; there is no process declassification sink and no authority argument in this phase, unlike the authorized R14 protected-HTTP-header sink.
- Recoverable failures use only the closed taxonomy documented in `GENIA_STATE.md` section 9.40; no raw Python exception detail, native executable path, or process identifier crosses into a returned context. `execution.process` adds no new Core IR node and no R35/R36 portable storage or location-independent execution semantics. This is Python-reference-host-only in this phase; shared/multi-host conformance evidence does not yet exist for this capability.
