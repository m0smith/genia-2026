...
- flow shared coverage is partial and limited to first-wave cases proving lazy pull-based observable behavior through early termination, single-use enforcement, deterministic outputs, `evolve(init, f)` progression, `refine(..steps)`, `rules(..fns)`, `step_*` / `rule_*` equivalence, `rules()` identity, focused Flow `map` / `filter` / `scan` behavior, selected Seq-compatible terminal behavior including `reduce` accumulation over Flow, and a resource lifecycle case (`seq-finalization-drop-take`) proving Flow-aware `drop |> take |> collect` composition with bounded pulling and correct output
...
- Callable behaviors:
  ...
  - exact/closed structural Templates are Experimental: `exact_shape_match({field: Template}, value)` requires an equal ordinary-map key set, preserves the original map, and distinguishes missing from extra fields without nominal Struct values
  ...
  - Runtime capability values:
    ...
    - runnable release examples and implemented-truth synchronization (Experimental R12 E12-8; documentation verification only) plus E12-9 release truth audit/distillation; R12 is release-complete without runtime changes
...
`recursive_template(name, build_fn, max_depth)` (**Experimental**, R15 E15-6, issue #733) builds a self-recursive Template through one explicit named reference. `build_fn(ref)` runs once at construction; `ref(name)` returns a self-reference Template while any other requested name always fails deterministically. Depth is tracked per instance (never a shared/global registry) and capped by `max_depth` (at most 100), so a self-reference beyond the bound fails with `err("recursive-template-depth-exceeded", {limit})` rather than ever risking a host `RecursionError`. `accumulate` and `template_schema` treat it as one leaf check at every level — a documented boundary, not a gap — while direct-call validation still performs real bounded recursive traversal over ordinary tree-shaped data.
...
syntax, or Core IR node. See [the R12 release page](docs/releases/R12.md).
...
E12-7 boundary. E12-9 also adds no runtime behavior; its truth audit makes R12
release-complete while the APIs remain Experimental, Python remains the only
...
R13 E13-4 adds `config_standard(overrides, args)` and its explicit-path three-argument form. They preserve four fixed indices and precedence: overrides > normalized explicit arguments > environment > `.env`. Two arguments use optional `.env`; a supplied path is required. Construction returns the exact existing provider Outcome, snapshots once, and leaves ordinary/secret views and protected boundaries unchanged. E13-5 verifies cross-mode explicit construction, snapshot-before-use/no-refresh behavior, normalized sentinel-free failures, exact R10 protected authority/audit/sink behavior, and unchanged ordinary parse/Core IR forms without adding runtime semantics.

Run `genia examples/r13_validated_pipeline_proving_case.genia` for the E13-6 offline application proof. One conventional provider supplies distinct server, database, and metrics `PORT` values through qualified views, explicit `parse_int`, and a callable Template; existing record helpers produce clean values plus ordered diagnostics. The public result observes its credential only as `<protected>`. Python reference-host tests inject the matching authority and outbound fixture to prove exactly one authorized reveal/audit/attempt, while mismatches and provider failures remain effect-free and non-revealing. E13-6 adds no API, runtime semantics, networking, retry, fallback, ambient lookup, or lifecycle behavior.
...
This E10-1/E10-7 surface has no ambient lookup, implicit conversion/coercion, new validation system, annotation injection, or new syntax/Core IR. File, command, pipe, import, native-test, and serve-entry evaluation retain the same explicit semantics: imports acquire only through explicit calls, serve snapshots finish before listener activation, and requests do not refresh configuration automatically.

Run `genia examples/r10_validated_pipeline_proving_case.genia` for the executable E10-7 composition proof. It validates ordinary endpoint/port configuration through existing Outcomes and callable Templates, aggregates clean records plus diagnostics with `validate_each`/`collect_validated`, and observes the credential only as `<protected>` plus a successful protected match. Python reference-host tests inject the matching authority and fixture outbound callable to prove explicit declassification immediately at that boundary; the public example has no real credential or network dependency.
...
- `json_schema(json_represented_schema) -> some(template, context) | err(reason, context)` — compile the supported JSON Schema structural subset into an ordinary Template (**Experimental**)
...
`json_decode` attaches exactly one outer `"json"` facet to an ordinary decoded root. A fraction/exponent JSON number token decodes to an exact Decimal (never a host binary64 float, R23 E23-3, issue #915). It rejects duplicate object names, invalid UTF-8/Unicode, integers outside ±9007199254740991, a Decimal/Rational/Float64 value failing the `stable_json_decimal` round-trip check or a non-finite Float64/`NaN`/`Infinity` spelling, malformed input, and nesting beyond 128 containers as recoverable `err` Outcomes. `json_encode` accepts that represented root or a supported ordinary value (Integer, a `stable_json_decimal` Decimal, a terminating-and-stable Rational, or a finite Float64) and emits sorted, two-space-indented JSON while preserving array order. The older parse/stringify/JSONL helpers remain compatibility surfaces, do not attach representation facets, and (R23 E23-5, issue #923) share the same lexical Decimal decode without the stability gate, plus Decimal/Rational/Float64 encode support using the same canonical digits.
...
- `validate_each(source, validator)` applies a callable validator to each item of a list or Flow source and returns one Outcome per item (**Experimental**, issue #392, issue #415, issue #416):
  ...
  - classifies each source item before invoking the validator: upstream `err(...)` and `none(...)` items are preserved unchanged without calling the validator; upstream `some(payload)` items have the validator called with the unwrapped `payload`; plain records and values are passed to the validator directly
...
- data parsing helpers: `json_decode`, `json_encode`, `json_schema` (Experimental portable boundaries), `json_parse`, `json_stringify`, `json_pretty`, `parse_jsonl_record` (Experimental), `parse_csv_row` (Experimental)
