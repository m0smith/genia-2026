...
- Only Python is implemented as a production host; all other hosts are planned or scaffolded only, and `m0smith/genia-cpp` is a bootstrapped repository shell with no real interpreter (R16 E16-6, issue #763).
- No browser runtime or playground is implemented; browser artifacts are documentation only.
- A generic multi-host runner exists (`tools/spec_runner --host`, R16 E16-1 through E16-7), but no real second production host is implemented yet; all conformance evidence to date is about the Python reference host or non-semantic proof fixtures.
...
- AI model invocation, Flow conversation composition, validated-pipeline proof, release-example truth sync, and release truth audit (Experimental, R11 E11-1 through E11-8, issues #611-#618)
  - `examples/ollama_chat.genia` is application code over the existing R10/R13/R14 protected-configuration and outbound HTTP surfaces; one shared terminal loop selects local Ollama `/api/chat` or Groq's OpenAI-compatible chat endpoint through named Value Templates and separate backend clauses of a three-function R20 open interface (profile, operation, content extractor), while generic response handling derives backend-specific diagnostic names from the backend value; `hosts/python/exec_ollama_chat.py` remains the narrow Python-host launcher that constructs the matching `chat_outbound` authority for the protected complete `GROQ_AUTHORIZATION` header; this adds no Ollama/Groq provider and does not change the R11 `model/4` boundary
