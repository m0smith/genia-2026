...
- `recursive_template(name, build_fn, max_depth)` builds a self-recursive Template through one explicit named reference; `name` must be a non-empty ordinary string, `build_fn` must be Template-callable, and `max_depth` must be a positive integer no greater than 100 (a conservative ceiling that keeps this operation's own explicit bound always firing well inside Python's default recursion limit — it never relies on `RecursionError`), or construction raises a clear `TypeError`.
