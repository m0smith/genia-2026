...
- R16 E16-1 (issue #758) implements the versioned host-adapter wire protocol (request/response JSON envelope, stdout/stderr channel separation, and the deterministic ok/unsupported/protocol-error/crash/timeout outcome taxonomy) as a standalone module, `tools/spec_runner/protocol.py`, proven only against the deterministic fixture adapter in `tools/spec_runner/fixtures/protocol_fixture_adapter.py`.
- R16 E16-2 (issue #759) adds `python -m tools.spec_runner --host '<command>'`: a generic external-host execution path that runs applicable discovered cases through the E16-1 protocol and reports the full six-state taxonomy. It is proven against the deterministic fixture adapter (including running the entire real `spec/` suite through it as a transport/taxonomy proof, with `passed=0` since the fixture is not a Genia host). **Without `--host`, the default in-process Python path is unchanged.** Cases requiring an injected Python-host-only test fixture or `--debug-stdio` are reported `unsupported` locally. The statement below still holds: no real second host exists.
- R16 E16-3 (issue #760) adds a mandatory `capabilities` protocol operation (`tools/spec_runner/protocol.py::fetch_capabilities`) and an optional per-case `requires:` field in spec YAML files. `--host` mode now fetches and validates the host's capability declaration once per run, before executing any case; a case with `requires` is applicable only when every listed capability is declared exactly `supported`, and an unmet requirement is reported `unsupported` without invoking the adapter for that case. An unknown claimed or required capability name (outside `spec/manifest.json`'s vocabulary) is a deterministic error that stops the run. No shared spec case currently declares `requires`; this adds the mechanism, not new coverage.
- R16 E16-4 (issue #761) adds `tools/spec_runner/revision.py`. `--host` mode classifies the host's declared `contract_revision` against the revision this checkout is actually at, using local git history only (never a remote fetch, never rewriting the host's claim): an exact match prints `Revision: pinned conformance for <sha>`; a resolvable older commit prints `Revision: current-main compatibility only -- ...` and proceeds (a host may be correctly conforming to its declared revision while visibly behind current `main`); an unresolvable declared revision stops the run with exit code 1 before any case executes. Genuinely re-running the suite pinned at an older revision's own historical `spec/` snapshot remains an external-host-CI concern (E16-7, issue #764).
- R16 E16-5 (issue #762) adds `hosts/python/protocol_adapter.py`, exposing the Python reference host itself as an E16-1 protocol-speaking subprocess command (`python -m tools.spec_runner --host 'python -m hosts.python.protocol_adapter'`), wrapping `hosts/python/adapter.py::run_case` with no change to evaluation semantics. Proven at full scale: the entire real `spec/` suite run through this subprocess path produces results identical to the in-process default path for every currently applicable case (641 total, 623 passed, 0 failed, 18 unsupported, 0 protocol_error/crash/timeout). The in-process default path (used when `--host` is omitted) is retained unchanged as a developer-optimization path; it is not deleted and is not the conformance definition.
...
- R16 E16-7 (issue #764) adds `--evidence <path>` to `tools/spec_runner --host`, writing one deterministic JSON evidence document (`tools/spec_runner/evidence.py`) per run: contract revision (declared, checkout, and E16-4 classification), protocol version, claimed capabilities, applicable-case count, and the full `pass`/`fail`/`unsupported`/`protocol_error`/`crash`/`timeout`/`invalid` taxonomy count. `build_evidence` is a pure function of its inputs (identical inputs produce byte-identical evidence) and raises rather than publish a document whose counts do not sum to the total. Proven against both the Python reference host (`hosts/python/protocol_adapter.py`: 623 pass / 18 unsupported / 0 else, revision classified `current`) and the `m0smith/genia-cpp` bootstrap placeholder (641 unsupported / 0 else, revision classified `resolvable_ancestor`) — the same evidence model honestly distinguishes pinned conformance from current-main-compatibility-only. See `docs/strategy/roadmap/multi-host-conformance-policy.md`'s "Evidence model and CI expectations" for the external-host CI contract this defines.
