"""Shared-spec inventory/execution contract for E15-4 (#731) template_schema."""
...
REQUIRED = {
    "eval": {
        ...
        "template-schema-shape-of-json-schema-fields",
        "template-schema-nested-shape",
        "template-schema-unsupported-opaque",
        "template-schema-unsupported-refinement-field",
        "template-schema-unsupported-default-field",
    },
    "error": {
        "error-template-schema-non-callable",
    },
}


def test_issue_731_template_schema_shared_spec_inventory_is_present():
...
def test_issue_731_template_schema_shared_specs_execute_as_contract():
    failures_by_name = {}
    for category, names in REQUIRED.items():
