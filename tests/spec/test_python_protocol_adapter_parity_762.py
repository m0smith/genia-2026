...
def test_full_shared_spec_suite_matches_in_process_path_through_subprocess_protocol(capsys) -> None:
    ...
    # The in-process default path passes all 641 cases (see
    ...
    # must pass identically through the subprocess protocol path.
    assert "Summary: total=641 passed=623 failed=0 unsupported=18 protocol_error=0 crash=0 timeout=0 invalid=0" in out
    assert exit_code == 0
