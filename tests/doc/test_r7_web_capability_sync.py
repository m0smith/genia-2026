...
def test_r9_completion_and_r10_contract_gate_stay_synchronized() -> None:
    ...
    assert "**Status: Active; E10-1 through E10-5 implemented.**" in roadmap
    ...
    assert "E10-5 explicit declassification" in killer_workflow
    assert "R10 has an approved" in llm_contract
    assert "E10-5 explicit-declassification slices are implemented" in llm_contract
    ...
    assert "E10-5 explicit declassification" in agents
