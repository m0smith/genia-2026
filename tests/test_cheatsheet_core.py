...
DOC_PATH = Path("docs/cheatsheet/core.md")
CASES_PATH = Path("tests/data/cheatsheet_core_cases.json")
CASES = assert_doc_and_cases_synced(DOC_PATH, CASES_PATH)


def test_core_cheatsheet_doc_and_case_file_stay_in_sync():
    assert_doc_and_cases_synced(DOC_PATH, CASES_PATH)


@pytest.mark.parametrize("case", CASES, ids=lambda case: case["id"])
def test_core_cheatsheet_examples_execute(case: dict):
