...
def test_ollama_chat_native_behavior_tests_pass():
    ...
    assert suite.get("total") == 9
    assert suite.get("passed") == 9, problems
...
def test_direct_ollama_main_uses_fallbacks_without_authority_leak(capsys, monkeypatch):
...
def test_groq_chat_declassifies_only_at_authorized_host_boundary(monkeypatch):
    ...
    values = GeniaMap().put("GROQ_AUTHORIZATION", sentinel)
