...
approved and E13-1 qualified views, E13-2 explicit CLI adaptation, and E13-3 narrow `.env` snapshots are implemented.** Its approved boundary is ordinary callable
...
existing literal source descriptor without acquiring process state; E13-3 reads one exact explicit path once with deterministic parsing and no discovery, interpolation, or refresh. Conventional
composition, hardening, proving, and release-close slices remain gated. R13
