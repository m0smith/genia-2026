...
- The identical eight requests pass through the R16 Python subprocess adapter
  when `open_functions` is `supported` (`pass=8`, all other outcomes zero).
- With `open_functions` declared `unsupported`, all eight are classified
...
- Focused loader, protocol, host-executor, Python-adapter, R20 shared/unit, and
  documentation tests pass (218 tests).
...
- **Capability honesty:** every new case requires `open_functions`; unmet
  requirements are unsupported without process invocation and never pass.
