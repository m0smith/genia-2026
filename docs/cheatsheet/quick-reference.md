...
- `web.cors(policy, handler)` wraps a handler with configured CORS headers and automatic true-preflight responses
...
- External direct process execution (Python-host-only, Experimental): `import execution` then `execution.process(capability, request)` where `capability` is an opaque host-created value (no Genia constructor) and `request` is `{executable: symbol, args: [string, ...], timeout_ms: integer}`. No shell, no `PATH` search; nonzero exit is `some({exit_code, stdout, stderr})`, never a failure; `stdout`/`stderr` are independently bounded `Bytes` at `1,048,576` bytes each; a protected request value is rejected before any attempt (no declassification sink). See `GENIA_STATE.md` section 9.40.
