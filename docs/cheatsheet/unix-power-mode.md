...
- Minimal validation helpers return `some(record)` for valid map records and recoverable `err(...)` diagnostics for missing or invalid fields. Validation `field` arguments may be flat names or simple dot-joined nested paths such as `"patient.name"` for helper lookup and diagnostic metadata only. `validate_record(record, validators)` (**Experimental**) composes multiple field validators over one record: success returns `some(clean_record)`; failure returns `err(quote(record_validation_failed), context_with_diagnostics)`. `validate_each(source, validator)` (**Experimental**) applies a validator to each item of a list source and returns one Outcome per item; list input is implemented and tested in this phase; Flow input and `validate_each/3` are not implemented in this phase. Use `collect_validated(results)` (Experimental) to collect a finite list or Flow of Outcome items into `{clean: [...], diagnostics: [...]}` for multi-record report collection.
