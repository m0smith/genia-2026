...
| JSON Schema validation | `json_schema(json_represented_schema)` — compile the supported structural subset into an ordinary Outcome Template (**Experimental**) |
| record validation | `validate_required`, `validate_field`, `validate_optional` for one map record at a time; `validate_record(record, validators)` to compose multiple field validators (**Experimental**) |
...
| diagnostic values | `diagnostic_error`, `diagnostic_skipped`, `diagnostic_reason`, `diagnostic_field` for explicit field/index-aware maps (**Experimental**) |
...
- `parse_jsonl_record(line)` (**Experimental**) parses one JSONL string line into an Outcome: valid JSON objects → `some(record, ctx)`, blank lines → `none("blank_line", ctx)`, malformed or non-object JSON → `err(reason, ctx)`. Every recoverable Outcome context includes the exact original input string as `line: <original_line>`. Non-string input is a runtime error. Does not change `json_parse`.
- `json_schema(json_represented_schema)` (**Experimental**) compiles the closed `type`/`properties`/`required`/`items`/boolean-`additionalProperties` subset into the existing callable Outcome Template model. Unsupported keywords fail explicitly; no coercion, defaults, references, or full JSON Schema conformance is implied.
- [`examples/r9_composed_json_template_pipeline.genia`](../../examples/r9_composed_json_template_pipeline.genia) is the directly tested composed pipeline: JSON boundary → represented-value match → JSON Schema-derived Template → `validate_each` → `collect_validated`.
- Minimal validation helpers return `some(record)` for valid map records and recoverable `err(...)` diagnostics for missing or invalid fields. Validation `field` arguments may be flat names or simple dot-joined nested paths such as `"patient.name"` for helper lookup and diagnostic metadata only. `validate_record(record, validators)` (**Experimental**) composes multiple field validators over one record: success returns `some(clean_record)`; failure returns `err(quote(record_validation_failed), context_with_diagnostics)`. `validate_each(source, validator)` (**Experimental**) applies a validator to each item of a list or Flow source and returns one Outcome per item; upstream `err(...)` and `none(...)` source items are preserved unchanged without calling the validator; upstream `some(payload)` items have the validator called with the unwrapped payload; plain records are passed to the validator directly; list input returns a list of Outcome values; Flow input returns a lazy Flow of Outcome values; `validate_each/3` is not implemented in this phase. Use `collect_validated(results)` (Experimental) to collect a finite list or Flow of Outcome items into `{clean: [...], diagnostics: [...]}` for multi-record report collection.
- `diagnostic_error(index, field, reason, context)` and `diagnostic_skipped(...)` create ordinary `{index, field, kind, reason, context}` maps; `diagnostic_reason` and `diagnostic_field` inspect maps using existing lookup behavior. They preserve caller values and do not automatically change `collect_validated` output.
