...
**R8 lifecycle core:** The Python reference host has an Experimental internal server-lifecycle coordinator that is explicitly callable with injected activation, request, and close operations without CLI parsing or sockets. Its inert descriptor and result data follow `GENIA_STATE.md` section 9.7. It adds no public Genia surface: server annotations, `genia serve`, and live lifecycle-to-HTTP integration remain unavailable.
