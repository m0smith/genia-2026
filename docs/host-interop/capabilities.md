...
- **notes:** The Python host snapshots `os.environ`. Portable provider semantics allow a future host to advertise an equivalent capability or return normalized unavailable; this row records implementation status today. No environment access occurs during lookup; `config_standard` invokes this existing capability at fixed source index 2 and adds no capability. E13-5 verifies construction-before-use and no refresh through views, imports, native-test seams, and serve startup without adding a capability. Literal `{kind: quote(values), values: map}` sources are portable ordinary values and require no host capability.
...
- **portability:** `Python-host-only`
- **notes:** The Python host reads bytes from exactly the supplied path. `config_standard` uses this existing capability at fixed source index 3: optional `.env` for two arguments or a required explicit path for three. Observable descriptor, grammar, failure, ordering, and snapshot obligations are portable. E13-5 verifies exact normalized source failure, sentinel non-leakage, snapshot-before-use, and no request-time refresh without adding a capability. Paths, keys, values, source content, and raw host details never appear in normalized failures; lookup performs no later host access.
...
**R8 lifecycle and annotation-binding internals:** The Python reference host has an Experimental internal server-lifecycle coordinator that is explicitly callable with injected activation, request, and close operations without CLI parsing or sockets. It validates inert `@route` metadata, discovers entry-file route handlers deterministically, and assembles existing generic R7 route values through `route_request`. It validates and normalizes inert `@server` metadata, requires exactly one entry-file owner, and passes a diagnostic-free configuration once to an injected operation with the existing `serve_http(config, handler)` shape. It also validates optional inert `@cors` metadata on that owner and wraps a diagnostic-free assembled handler at most once through the existing `cors(policy, handler)` shape. Discovery and binding do not start a listener or execute handlers. These descriptor/result data follow `GENIA_STATE.md` section 9.7. `genia serve <file>` is implemented and is the only mode that activates this lifecycle; load, import, parse, evaluation, and discovery remain inert in every other mode. R14 (E14-10, issue #627) further proves that an ordinary route handler invoked this way may create outbound HTTP client lifecycle instances (`web.http_send`/`web.send_annotated`) while the server stays active — `server_lifecycle.py` and R14's `lifecycle_runtime.py` remain architecturally separate; see `GENIA_STATE.md` section 9.17.

**Not Guaranteed:** There is no public `options(...)` route, no header-only CORS API, and no extra `json`/`text` header arities. The current phase also provides no path-parameter routing, general middleware, streaming request/response bodies, WebSocket support, async or concurrent server, credentials policy, origin reflection/allowlist, per-route CORS override, or portability to non-Python hosts.
...
- **name:** `http.transport`
- **genia_surface:** none — this is a private Python-host capability with no Genia builtin, prelude entry, or `import` surface of its own. It is consumed internally by `web.http_send` (R14 E14-7, issue #624) and, through it, by `web.send_annotated` (R14 E14-9, issue #626); both perform `HttpOperation` declassification/encoding before calling this capability, and neither adds a second transport surface.
...
- **`spec/manifest.json` / R16 registration — registered, but not yet conformance-proven:** `execution_process` is registered in `spec/manifest.json`'s `optional_capabilities`, matching this repository's established rule that registration recognizes a name but does not prove shared coverage. `shell_stage` and `debugger_stdio` remain examples with no requiring shared cases; `process_primitives` is no longer such an example because R25 E25-0 supplies real cases. `tools/spec_runner/capabilities.py::known_capabilities()` reads the manifest as the sole source of recognized names, and `hosts/python/protocol_adapter.py::_python_claimed_capabilities()` automatically self-declares every recognized name `supported` (unless listed as `partial`). **What this registration does *not* mean:** no host-neutral shared-spec fixture/provisioning mechanism yet exists to let a portable `eval`-category case inject an external-process capability into the sandboxed evaluation, so no shared-spec case declares `requires: [execution_process]` yet, and the Python host's `supported` self-declaration is therefore not yet exercised or falsified by an applicable case. Runtime implementation, capability advertisement, and executable conformance evidence remain separate facts. A future host-neutral provisioning mechanism would be required to exercise this claim.
