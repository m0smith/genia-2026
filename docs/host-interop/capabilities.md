...
**R8 lifecycle and route-binding internals:** The Python reference host has an Experimental internal server-lifecycle coordinator that is explicitly callable with injected activation, request, and close operations without CLI parsing or sockets. It also validates inert `@route` metadata, discovers entry-file route handlers deterministically, and assembles existing generic R7 route values through `route_request` without starting a listener or executing handlers during discovery. Their descriptor/result data follow `GENIA_STATE.md` section 9.7. `@server`, `@cors`, `genia serve`, and live lifecycle-to-HTTP integration remain unavailable.
