...
These rows distinguish the implemented E14-1 foundation from later approved
R14 boundaries; see `r14-composable-lifecycle-contract.md`.
...
| peer lifecycle attachment | ordered `LifecycleDefinition` list / `LifecycleResult` | E14-1 uses list-order entry and reverse unwind only as required by explicit root/child scopes; E14-2 owns the broader peer-composition slice. Attachment order is not parentage and a peer cannot mutate another peer's owned context/state/resources | E14-1 foundation implemented; broader E14-2 planned |
...
| lifecycle context | ordinary value / explicit scope handle | context is read only through `lifecycle_context(handle, name)`, never injected into lexical bindings; a handle used outside its entering/active/exiting lifetime raises `lifecycle-scope-expired` | E14-1 implemented, Experimental |
