...
| configuration provider | Outcome / Template | explicit immutable lookup returns exact-string `some(...)` or missing `none(...)`; `config_get_or` lazily supplies only missing values; R13 `config_view`/`secret_view` qualify exact logical names through one unchanged lookup without adding defaults or validation; explicit converters return existing Outcomes, and callable Templates validate converted successes without a second schema or error model | R10 E10-1/E10-2 and R13 E13-1 implemented, Experimental |
