...
| accumulated diagnostics | `open_shape` / `exact_shape` / `refinement` / `default_field` / `json_schema` / Outcome | `accumulate(template, value)` collects every independent path-aware diagnostic instead of short-circuiting; deep recursion covers the `open_shape`/`exact_shape` family, other inspectable Templates are one leaf check; preserves `some`/`none`/`err` distinctions and existing pattern/Template direct-call semantics unchanged | Implemented R15 E15-3, Experimental |
| Template to JSON Schema | `json_schema` / `open_shape` / `exact_shape` / `template_description` | `template_schema(template)` is the bounded reverse of `json_schema`, generating a faithful schema through pure inspection of `template_description` data for `json_schema`-derived Templates and fully-convertible `open_shape`/`exact_shape` structures; an opaque Template, a bare `refinement` field, or a `default_field` fail deterministically rather than approximating | Implemented R15 E15-4, Experimental |
| structural alternatives | named matcher / `@?` / `@!` / `&` / `accumulate` / `template_description` / `template_schema` | `alternatives(discriminator, branches)` selects exactly one branch by an explicit string discriminator field and validates only that branch against the full subject; ordinary map/value payloads only, no nominal variant; composes with existing pattern/matcher/inspection operations automatically since it is an ordinary Template | Implemented R15 E15-5, Experimental |
| bounded recursive references | named matcher / `@?` / `@!` / `&` / `accumulate` / `template_description` / `template_schema` | `recursive_template(name, build_fn, max_depth)` builds a self-recursive Template through one explicit named reference resolved entirely within its own construction-time closure; depth is bounded per instance and never relies on host recursion limits; `accumulate`/`template_schema` treat it as one leaf check at every level | Implemented R15 E15-6, Experimental |
...
| configuration provider | Outcome / Template | explicit immutable lookup returns exact-string `some(...)` or missing `none(...)`; `config_get_or` lazily supplies only missing values; R13 `config_view`/`secret_view` qualify exact logical names through one unchanged lookup without adding defaults or validation; `config_args` purely normalizes explicit string arguments into the existing literal source descriptor, while an explicit `.env` descriptor and `config_standard` snapshot exact strings through the same provider before existing converters and callable Templates compose with lookup Outcomes; E13-5 verifies this composition across modes and protected boundaries, and E13-6 proves three qualified `PORT` Outcomes plus callable Template validation inside the existing validated-record pipeline without changing the boundary | R10 E10-1/E10-2 and release-complete R13 boundary, Experimental; E13-7 documentation verification and E13-8 audit/distillation only |
| R13 public examples | explicit provider / Outcome / Template / protected value | executable qualified-view and complete validated-pipeline examples verify the existing explicit snapshot, fixed-precedence, conversion/validation, and protected transport boundary without adding a helper, ambient lookup, or configuration framework | E13-7 documentation verification implemented; no runtime behavior |
...
E14-10 (#627) proves the "parent/child execution scope" and "outbound
HTTP operation/client" rows below compose correctly when an R8 route
...
E14-13 (#696) proves that every row below holds at combined
cross-cutting breadth (import/discovery inertness, sentinel-free
rendering together, combined multi-peer failure ordering, bounded Flow
termination with no leak, and server/request/outbound-client
resilience) — a behavioral hardening proof over the existing rows,
adding no new Genia value relationship and no new row; see
`GENIA_STATE.md` section 9.20.

| Concept | Composes with | Required relationship | Status |
|---|---|---|---|
| parent/child execution scope | ordinary callable / `LifecycleResult` | `lifecycle_child` runs a fully synchronous nested entry/work/unwind algorithm and always returns an ordinary result value to the parent's `work`; propagating a child failure is the parent's explicit choice, never automatic | E14-1 implemented, Experimental |
| peer lifecycle attachment | ordered `LifecycleDefinition` list / `LifecycleResult` | peers on one scope enter in list order and unwind in strict reverse order; attachment order is not parentage and a peer cannot mutate another peer's owned context/state/resources | E14-2 implemented, Experimental |
...
| Accumulated diagnostics | `open_shape` / `exact_shape` / `refinement` / `default_field` / `json_schema` / Outcome | `accumulate(template, value)` collects every independent field/index diagnostic in one call instead of short-circuiting; deep depth-first recursion covers the `open_shape`/`exact_shape` family (replicating each's own contracted missing/extra/field-validation phase order), while a bare `refinement` or other inspectable Template is one leaf check; each diagnostic's `reason` forwards only the underlying Outcome's own reason, never its context, so protected/represented payloads cannot leak through; zero diagnostics delegates to the real Template call for the success value; `accumulate` changes no pattern-dispatch, `@?`/`@!`/`&`, or Flow bounded-demand semantics | E15-3 (issue #730) implemented, Experimental |
| Template to JSON Schema | `json_schema` / `open_shape` / `exact_shape` / `template_description` / R9 `json` representation | `template_schema(template)` reverses `json_schema` through pure inspection of `template_description` data: a `json_schema`-compiled Template's own description converts directly, and `open_shape`/`exact_shape` convert only when every field recursively converts; an opaque Template, a bare `refinement` field, or a `default_field` fail deterministically (`err(quote(unsupported-template), {path, kind})`) rather than approximating, since a refinement's predicate is never introspected/executed and JSON Schema's `default` keyword cannot represent missing-only insertion semantics; round-trip claims (`json_schema` -> `template_schema` -> `json_schema`) are limited to accept/reject equivalence over the tested subset | E15-4 (issue #731) implemented, Experimental |
| Structural discriminated alternatives | `open_shape` / `exact_shape` / `refinement` / `json_schema` / named pattern / `@?` / `@!` / `&` / `accumulate` / `template_description` / `template_schema` | `alternatives(discriminator_field, branches)` reads one explicit string discriminator field, resolves exactly one branch from a closed map, and validates only that branch against the full unmodified subject; missing/invalid/unknown discriminators produce deterministic `none(...)` diagnostics and no other branch is ever attempted; it is inspectable (`template_description`), recurses through `accumulate`, and is deterministically unsupported by `template_schema` (no discriminated-union JSON Schema primitive); it composes with named patterns, `@?`/`@!`/`&`, and `Name(inner)` automatically since it is built the same way as every other R15 Template builder; only #92's structural-validation portion is absorbed — nominal variant identity, constructors, sealed hierarchies, and exhaustiveness remain deferred | E15-5 (issue #732) implemented, Experimental |
| Bounded named recursive Template references | `open_shape` / `exact_shape` / `alternatives` / named pattern / `@?` / `@!` / `&` / `accumulate` / `template_description` / `template_schema` | `recursive_template(name, build_fn, max_depth)` calls `build_fn(ref)` exactly once; `ref(name)` returns a self-reference Template validated with a dedicated per-instance depth counter (never shared/global) capped at `max_depth` (at most 100), while `ref(other_name)` always fails deterministically as an unresolved reference; exceeding the bound fails before ever invoking the wrapped structure, so no host recursion limit is ever relied upon; `accumulate`/`template_schema` treat it as one leaf check at every level, a documented boundary that avoids a second independently-bounded traversal inside either operation | E15-6 (issue #733) implemented, Experimental |
