...
| configuration provider | Outcome / Template | implemented explicit immutable provider lookup returns exact-string `some(...)`, missing `none(...)`, or normalized provider `err(...)`; defaults, conversion, and validation remain later ordinary-call/Template composition rather than a second schema or error model | R10 E10-1 implemented, Experimental; later composition contracted, not implemented |
