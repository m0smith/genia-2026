...
- **execution-process-contract.md** — approved, merged (PR #977) contract for the first bounded `execution.process` direct-execution slice, now **implemented** in the Python reference host; see `GENIA_STATE.md` section 9.40 for the authoritative implemented contract.
- **execution-process-design.md** — approved implementation plan for `execution-process-contract.md` (capability/provider model, symbolic resolution, validation ordering, ownership/timeout/capture design, failure normalization, protected-value handling, Python reference-host decomposition, and R16/R35/R36/R37 boundaries), now **implemented**; see `GENIA_STATE.md` section 9.40.
- **r20-open-functions-preflight.md** — E20-0 inventory and readiness gate for existing function groups, pattern dispatch, modules, Core IR, diagnostics, help metadata, and shared conformance.
- **r20-open-functions-contract.md** — implementation-ready R20 semantic contract for local clause accumulation and explicit, provenance-preserving cross-module contributions; surface syntax remains a later design decision.
