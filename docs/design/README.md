...
- **execution-process-contract.md** — proposed, unimplemented contract for the first bounded `execution.process` direct-execution slice; requires independent contract review before design or implementation.
- **execution-process-design.md** — design-phase-only implementation plan for the approved `execution-process-contract.md` (merged PR #977): capability/provider model, symbolic resolution, validation ordering, ownership/timeout/capture design, failure normalization, protected-value handling, Python reference-host decomposition, and R16/R35/R36/R37 boundaries. Not implemented; next phase is failing tests.
