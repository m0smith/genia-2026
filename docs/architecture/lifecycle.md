...
Lifecycle plan data-shape validation and normalization is implemented in the Python reference host as a focused internal utility (`src/genia/lifecycle_plan.py`, issues #449 and #451). `validate_lifecycle_plan(value)` and `normalize_lifecycle_plan(value)` validate that an ordinary map value conforms to the portable lifecycle plan data shape: required `name` and `phases` fields, required `name` and `action` identifier fields per phase, boolean `always` normalization, duplicate-phase-name rejection, and optional root policy maps named `cleanup`, `failure_policy`, and `result_policy`. Root policy maps normalize contract-safe defaults and reject unsupported, unsafe, or nonportable policy values such as unentered-scope cleanup, cleanup failures that overwrite or disappear, and nondeterministic failure reporting. This is Python reference-host data validation only; no public Genia prelude API, no lifecycle runner, no cleanup execution, no phase execution, no action resolution, and no annotation discovery were added. See `GENIA_STATE.md` section 9.3 for the full language contract.
