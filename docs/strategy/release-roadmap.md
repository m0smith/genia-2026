...
**R18 — Portable Value Equality** is complete. Epic **#789** covers
**#790 / E18-0** through **#797 / E18-7**; its approved contract, 24 shared
semantic cases, authoritative documentation, release page, and skeptical audit
are delivered. The audit verdict is PASS. It settles
...
**R19 — Unicode and Diagnostic Portability Contract is complete.** It settled
code-point string semantics, strict UTF-8 boundary/decode behavior,
deterministic debug escaping, and a diagnostic-wording leak audit. R19 does
not own float/Decimal/Rational semantics or Float64 rendering; that work is
split into a separate exact-numeric-model release gate. See
`docs/releases/R19.md`.

**R20 — Open Functions and Extensible Pattern Dispatch is complete.** It adds
one identity-bearing open function interface concept: local repeated/grouped
pattern clauses accumulate deterministically, and separately authored
modules extend an interface only through explicit `extend`/`use` contribution
and selection — never through ordinary import. See `docs/releases/R20.md`.
