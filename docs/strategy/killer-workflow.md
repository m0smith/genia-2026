...
**R14** is in progress through E14-6. It is explicitly approved lifecycle
...
transformations remain authoritative. E14-0 through E14-6 (issues #620, #621,
#692, #693, #694, #622, #623) implement and prove the Experimental parent/child
...
common inert `HttpOperation` representation with zero network IO, and one
narrow Python-host outbound HTTP transport capability with no Genia-visible
surface of its own; E14-7 and later slices — the outbound HTTP client
lifecycle and application-integration proof — remain planned, not
implemented.
