...
- **Includes:** local grouped/repeated open-function clauses; declaring `open_functions` `supported` (see acceptance-criteria annotation below — `tools/spec_runner/capabilities.py`'s requires-gate grants zero evidence credit for `partial`, only `supported` unlocks `requires: [open_functions]` cases at all; this is not a claim that cross-module R20 is implemented, see `#973`). Cross-module `open`/`extend`/`use` contribution and selection describes R20 the language contract, not this ticket's achievable `genia-cpp` scope — see `m0smith/genia-2026#971`.
...
- **Affected docs/tests/specs:** none in `genia-2026`.
- **Acceptance criteria:** `open_functions_r20` category of `bootstrap-cases.json` (2 cases; the cross-module case was replaced by `m0smith/genia-2026#971` with the repeated-clause sibling of the grouped-clause case, since the original required `multi_file_eval`) passes; capability declared `supported` after passing (`#973`: `partial` would make these two `requires`-gated cases permanently un-runnable by `tools.spec_runner`, which only attempts a `requires`-gated case when every listed capability is declared exactly `supported` — per-case correctness for out-of-scope R20 surface, e.g. cross-module dispatch, the R20 diagnostic family, bare varargs patterns, is still independently enforced and must show `unsupported`, never a wrong `ok`).
...
- **Required phases:** failing evidence -> minimal implementation -> capability declaration -> docs sync.
- **Prerequisites:** E24-4.
- **Capability/case evidence targeted:** `bootstrap-cases.json` category `open_functions_r20` (2 cases, both single-file/local as of `#971`); wider `spec/eval/r20-*.yaml`/`spec/ir/r20-*.yaml`/`spec/error/error-r20-*.yaml` families — including every `r20-cross-module-*.yaml` case, all of which require `multi_file_eval` and stay gated out on that separate requirement regardless of `open_functions`'s declared status — remain future follow-up evidence, gated on a roadmap-level decision to bring `multi_file_eval` into R24 scope, before `genia-cpp`'s `open_functions` implementation covers the full R20 contract (cross-module contribution/selection and the R20 diagnostic family).
