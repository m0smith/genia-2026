# R22–R25 — C++ Host Expansion and Sheet Record Pipelines

Status: Planned roadmap detail — non-authoritative. `GENIA_STATE.md` remains final authority for implemented behavior.

C++ production implementation for R22–R24 is planned to live in
...
## Release R22 — C++ Stateful Runtime and Concurrency

**Status: Planned, not active.** R22 depends on R21. Any currently
...
- preserve only currently implemented lifecycle truth, including R4, R8, and
  ...
  9.8-9.20) and may be referenced by R22 work once R22 itself begins
...
## Release R23 — C++ REPL and Data Bridges

**Status: Planned, not active.** R23 depends on R21. Its tracks may be ticketed
...
- `-p`/`--pipe` and Flow phase 1; pipe mode is Flow-backed and belongs in R24
...
## Release R24 — C++ Flow, Pipe Mode, and HTTP Serving

**Status: Planned, not active.** R24 depends on R21; its HTTP track also depends
on any R22 stateful-runtime behavior the approved design actually requires, and
its data boundaries may consume R23. Flow and HTTP remain separately gated
...
## Release R25 — Sheet Record Pipelines

**Status: Planned, not active.** R25 is a focused killer-workflow release. Its
...
R25 builds on the current Experimental Sheet foundation: `rows`, `where`,
...
- compose that context with R14's repeated element scopes and peer lifecycle
  ...
  not landed, R25 must not implement a Sheet-specific substitute lifecycle
