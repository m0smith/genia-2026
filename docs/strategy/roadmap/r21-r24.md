# R21–R24 — C++ Host Expansion and Sheet Record Pipelines

Status: Planned roadmap detail — non-authoritative. `GENIA_STATE.md` remains final authority for implemented behavior.

C++ production implementation for R21–R23 is planned to live in
...
## Release R21 — C++ Stateful Runtime and Concurrency

**Status: Planned, not active.** R21 depends on R20. Any currently
...
- preserve only currently implemented lifecycle truth, including R4, R8, and
  ...
  9.8-9.20) and may be referenced by R21 work once R21 itself begins
...
## Release R22 — C++ REPL and Data Bridges

**Status: Planned, not active.** R22 depends on R20. Its tracks may be ticketed
...
- `-p`/`--pipe` and Flow phase 1; pipe mode is Flow-backed and belongs in R23
...
## Release R23 — C++ Flow, Pipe Mode, and HTTP Serving

**Status: Planned, not active.** R23 depends on R20; its HTTP track also depends
on any R21 stateful-runtime behavior the approved design actually requires, and
its data boundaries may consume R22. Flow and HTTP remain separately gated
...
## Release R24 — Sheet Record Pipelines

**Status: Planned, not active.** R24 is a focused killer-workflow release. Its
...
R24 builds on the current Experimental Sheet foundation: `rows`, `where`,
...
- compose that context with R14's repeated element scopes and peer lifecycle
  ...
  not landed, R24 must not implement a Sheet-specific substitute lifecycle
