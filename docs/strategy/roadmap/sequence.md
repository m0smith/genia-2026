...
R39 — Unified Events and Subscriptions
  (builds on completed R14 + existing Flow; local baseline is independent of R35/R36,
   while later resource/distributed providers reuse R35/R36)
```
...
## Unified events and subscriptions

R39 promotes the cross-cutting event/subscription architecture described in `docs/architecture/unified-events-subscriptions.md`. It reuses R14 lifecycle ownership and existing Flow composition rather than creating a second event-stream or lifecycle system. The first contract is local/live and bounded-demand; durability, replay, broker mechanics, actor mailboxes, and distributed realization remain separate capability/provider concerns. See `docs/strategy/roadmap/r39.md`.
