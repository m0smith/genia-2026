...
Alphabetical index of the **217** out-of-the-box Genia functions available from the autoloaded prelude. Every entry is generated from the `@doc` / `@category` metadata in the source -- see [the @doc Style Guide](../style/doc-style.md). Do not edit these pages by hand.

!!! note
    Host built-in functions (e.g. `print`, `parse_int`) are documented via a
    host doc registry in a follow-up phase and will appear here once available.
