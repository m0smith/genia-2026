...
if __package__ in (None, ""):
    ...
    from genia.retrieval import construct_chunks, construct_embed, construct_index, construct_retrieve
    ...
    from .retrieval import construct_chunks, construct_embed, construct_index, construct_retrieve
...
def make_global_env(
    ...
    def refinement_match_fn(predicate: Any, value: Any) -> Any:
        if not _template_callable(predicate):
            raise TypeError(
                "refinement_match expected predicate function, "
                f"received {_runtime_type_name(predicate)}"
            )
        result = _invoke_raw_from_builtin(predicate, [value])
        if not isinstance(result, bool):
            raise TypeError(
                "refinement_match predicate must return bool, "
                f"received {_runtime_type_name(result)}"
            )
        if result:
            return GeniaOptionSome(value)
        return make_none("refinement-mismatch")

    def open_shape_match_fn(fields: Any, value: Any) -> Any:
        if not isinstance(fields, GeniaMap):
            raise TypeError(
                "open_shape_match expected field Templates map, "
        ...
        for field, template in fields.items():
            if not isinstance(field, str):
                raise TypeError(
                    "open_shape_match expected string field name, "
                    f"received {_runtime_type_name(field)}"
                )
            if not _template_callable(template):
                raise TypeError(
                    f"open_shape_match field {field} expected Template function, "
        ...
        for field, template in fields.items():
            ...
            if not isinstance(result, (GeniaOptionSome, GeniaOptionNone, GeniaOptionErr)):
                raise TypeError(
                    f"open_shape_match field {field} Template must return Outcome, "
                    f"received {_runtime_type_name(result)}"
                )
    ...
    def cors_fn(policy_value: Any, handler: Any) -> Callable[[Any], GeniaMap]:
        origin, methods, headers = resolve_cors_policy(policy_value)

        if not callable(handler):
            raise TypeError("cors expected handler to be callable")

        cors_headers = GeniaMap()
        cors_headers = cors_headers.put("access-control-allow-origin", origin)
        cors_headers = cors_headers.put("access-control-allow-methods", ", ".join(methods))
        cors_headers = cors_headers.put("access-control-allow-headers", ", ".join(headers))

        def cors_handler(request_value: Any) -> GeniaMap:
            request_method = request_value.get("method", "")
            request_headers = request_value.get("headers", GeniaMap())
            is_preflight = (
                request_method == "OPTIONS"
                and isinstance(request_headers, GeniaMap)
                and request_headers.has("origin")
                and request_headers.has("access-control-request-method")
            )
            if is_preflight:
                return GeniaMap().put("status", 204).put("headers", cors_headers).put("body", OPTION_NONE)

            response_value = _invoke_from_builtin(handler, [request_value])
            return with_headers_fn(cors_headers, response_value)
    ...
    env.set("embed", _host_function_group("embed", 4, construct_embed))
