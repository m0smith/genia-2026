...
- builtins:
  ...
  - R14 E14-1 adds Experimental explicit `lifecycle_scope`, synchronous `lifecycle_child`, and inward-only `lifecycle_context` calls; handles are opaque and lifetime-bounded, child results are ordinary data, and definitions/imports remain inert until explicit activation
  - E14-1 adds no ambient scope, repeated element/Flow lifecycle, lifecycle-owned provider, outbound HTTP, generalized R4 plan runner, concurrency, cancellation, syntax, or Core IR node
