...
- runtime value categories today:
  ...
  - runtime capability values:
    ...
    - Experimental R12 E12-1 `chunk/2` ordinary behavior validates exact documents and spans, slices original text by Unicode code points, and constructs exact provenance while preserving R9 represented metadata; it uses no provider capability
    - Experimental R12 E12-2 `embed/4`, E12-3 `index/4`, E12-4 `retrieve/4`, and E12-5 `rerank/4` return ordinary callables over explicit opaque Python fixture capabilities; E12-6 adds an importable application module that assembles exact grounded context/answers, preserves evidence order, derives first-occurrence exact sources, propagates non-success model Outcomes, and invokes a supplied unchanged R11 model callable once; E12-7 adds explicit cross-mode hardening plus a complete Outcome-aware grounded proving example without new semantics; E12-8 synchronizes runnable release examples and public truth, and E12-9 completes audit/distillation without runtime changes; R12 is release-complete, its APIs remain Experimental, and citation presentation remains unimplemented
    - portable capability values implemented by Python and C++: Ref, Cell, and local Process handle (R25 shared-evidenced)
    - Python-host-only capability values: Bytes wrapper, Zip entry wrapper, blocking HTTP server bridge
...
- literals: numbers, strings (single/double quotes + escapes, plus triple-quoted multiline strings), booleans, legacy `nil`, `none`
  - numeric literals classify their source as Integer (`DIGIT+`) or
  ...
  - since R22, Integer source materializes to arbitrary-precision Integer
    (unchanged) and Decimal source materializes to an exact runtime
    `GeniaDecimal`, never a host binary float; `rational(n, d)`,
    `float64(value)`, and `exact(value)` construct/convert the runtime
    Rational and explicit Float64 domains — see `GENIA_STATE.md` sections
    9.23–9.31 and `docs/releases/R22.md`
...
- pattern matching:
  ...
  - refinement/open structural Templates — Experimental; `refinement_match(predicate, value)` lifts a boolean predicate, while `open_shape_match({field: Template}, value)` requires listed map fields, allows and preserves extras, and propagates nested mismatch/error Outcomes
  - exact/closed structural Templates — Experimental; `exact_shape_match({field: Template}, value)` requires precisely the listed ordinary-map field set, preserves the original map, and returns distinct missing/extra mismatch Outcomes without Struct objects or layout promises
  - inert inspectable Template descriptions — Experimental, R15 E15-1 (issue #728); `refinement(predicate)`, `open_shape(fields)`, and `exact_shape(fields)` are curried one-argument Template builders behaving identically to their `*_match` counterparts, and `template_description(template)` returns `some(description)` for a Template built this way (or by `json_schema`) and `none("opaque-template")` for any other callable/named-pattern Template; descriptions are inert ordinary data with no effect on identity, matching, or `@?`/`@!`/`&`
  - explicit missing-only field defaults — Experimental, R15 E15-2 (issue #729); `default_field(default, template)` wraps a field Template used inside `open_shape`/`exact_shape` so a missing field validates and inserts `default` while a present field is always validated by `template` and never falls back; a shape with no applied defaults returns the original subject unchanged, and `template_description` records only that a default exists, never its literal value; explicit normalization is ordinary pipeline composition, not a new builtin
  - accumulated path-aware validation diagnostics — Experimental, R15 E15-3 (issue #730); `accumulate(template, value)` validates against an inspectable Template and collects every independent field/index failure (deep for `open_shape`/`exact_shape`, one leaf check otherwise) as `{path, kind, reason}` entries, never carrying the underlying Outcome's context; zero diagnostics returns the real Template success value, otherwise `err(quote(accumulated-validation-failed), {diagnostics})`; no change to pattern dispatch, `@?`/`@!`/`&`, or Flow bounded-demand semantics
  - faithful Template to JSON Schema generation — Experimental, R15 E15-4 (issue #731); `template_schema(template)` reverses `json_schema` through pure inspection over `template_description` data, never invoking the Template or a refinement predicate; a `json_schema`-derived Template and any `open_shape`/`exact_shape` of fully-convertible fields generate a faithful schema, while an opaque Template, a bare `refinement` field, or a `default_field` fail deterministically as `err(quote(unsupported-template), {path, kind})` rather than approximating
  - structural discriminated alternatives — Experimental, R15 E15-5 (issue #732); `alternatives(discriminator, branches)` selects exactly one branch by an explicit string discriminator field and validates only that branch against the full subject, with deterministic `none(...)` diagnostics for missing/invalid/unknown discriminators and no speculative other-branch attempts; composes with patterns/`@?`/`@!`/`&`, `accumulate`, and `template_description` automatically, and is deterministically unsupported by `template_schema`; only #92's structural-validation portion is absorbed, nominal variant identity/constructors/exhaustiveness remain deferred
  - bounded named recursive Template references — Experimental, R15 E15-6 (issue #733); `recursive_template(name, build_fn, max_depth)` builds a self-recursive Template through one explicit named reference resolved entirely within its own construction-time closure (no global registry, config, or lifecycle state), with a dedicated per-instance depth counter capped at `max_depth` (at most 100) so a bound-exceeding self-reference fails deterministically rather than ever risking a host `RecursionError`; `accumulate`/`template_schema` treat it as one leaf check at every level, a documented boundary rather than a gap
  - carrier representations — Experimental; `represent(facet, value)` adds one ordered outer facet, `representation_match(facet, value)` returns `some(carried)` or `none("representation-mismatch")`, and `strip_representation(facet, value)` explicitly removes one matching outer layer
  - representation-aware matching reuses named Templates: `pattern Name(value) = representation_match("facet", value)` enables `Name(inner)` without new syntax or a second matcher namespace
  - represented values transport unchanged through ordinary calls, collections, pipelines, Seq, Flow, and Sheet cells; derived values do not inherit facets implicitly; generic display/debug output is opaque as `<represented>`
...
- autoloaded stdlib functions keyed by `(name, arity)`
  ...
  - includes public R25-portable ref helpers from `src/genia/std/prelude/ref.genia`: `ref`, `ref_get`, `ref_set`, `ref_is_set`, `ref_update` (Python and C++ implemented/evidenced)
  - includes public R25-portable local process helpers from `src/genia/std/prelude/process.genia`: `spawn`, `send`, `process_alive?` (Python and C++ implemented/evidenced)
  ...
  - includes R25-portable `cell_primitives` helpers `cell`, `cell_with_state`, `cell_send`, `cell_get`, `cell_state`, `cell_failed?`, `cell_error`, `restart_cell`, `cell_status`, `cell_alive?` (Python and C++ implemented/evidenced)
...
- builtins:
  ...
  - first rendering/Representation System entry points (separate from carrier representations):
  ...
  - R13 E13-6 proves the existing Outcome-aware validated-pipeline composition, E13-7 synchronizes runnable release examples and public truth without runtime changes, and E13-8 completes audit/distillation without runtime changes; R13 is release-complete and remains Experimental, Python is the only implemented host, and shared/multi-host conformance remains Partial
  - R14 is complete through E14-15: `lifecycle_scope`/`lifecycle_child`/`lifecycle_context`/`lifecycle_repeat`/`lifecycle_config` are an Experimental parent/child/peer/repeated-element execution-scope core over ordinary calls and values, with no new syntax; `http_operation`/`web.http_send` add one inert closed HTTP-operation value and one narrow Python-host outbound transport capability, with a protected value in `headers` declassified only immediately before the one transport attempt; `@get`/`@post` are inert descriptor metadata activated only by the explicit `web.send_annotated` call; an R8 route handler may call either any number of times per request while the server stays active; E14-13 proves the whole combined surface holds at cross-cutting breadth (import/discovery inertness, sentinel-free rendering, combined failure ordering) with zero additional runtime code; and E14-14/E14-15 completed documentation synchronization and a **PASS** release truth audit with no runtime behavior — R14 is release-complete and remains Experimental, Python is the only implemented host, and shared/multi-host conformance remains Partial; see `docs/releases/R14.md` and `GENIA_STATE.md` sections 9.8-9.20
  ...
  - R25-portable ref runtime helpers are exposed publicly through prelude-backed wrappers: `ref`, `ref_get`, `ref_set`, `ref_is_set`, `ref_update` (Python and C++ implemented/evidenced)
  - R25-portable local process runtime helpers are exposed publicly through prelude-backed wrappers: `spawn`, `send`, `process_alive?` (Python and C++ implemented/evidenced; Actor excluded)
  ...
  - minimal record validation helpers are exposed publicly through prelude-backed wrappers: `validate_required(field, record)` and `validate_field(field, predicate, expected, record)` return `some(record)` for valid map records and recoverable `err(...)` diagnostics for missing/invalid fields; `field` may be a flat name or simple dot-joined nested path such as `"patient.name"` for validation helper lookup and diagnostic metadata only; non-callable predicates remain runtime errors
  - validation diagnostic fields are Experimental and stable per producing layer rather than one universal schema: missing-field helper context has `field` and `reason`; invalid-field helper context also has `expected` and `actual`; either has `row` only when the input record contains it; `validate_record` field diagnostics have `field`, `status`, `reason`, and nested `context`, with outer `diagnostics`; `collect_validated` aggregate diagnostics have zero-based `index`, `kind`, `reason`, and wrapped `context`
  - `diagnostic_error(index, field, reason, context)` and `diagnostic_skipped(index, field, reason, context)` create ordinary field/index-aware maps with exact keys `index`, `field`, `kind`, `reason`, and `context` (**Experimental**, issue #393); caller values are preserved without coercion or context wrapping, including `none(...)`; `diagnostic_reason(diagnostic)` and `diagnostic_field(diagnostic)` reuse existing map lookup behavior; these helpers do not alter or automatically integrate with existing validation diagnostic layers
  ...
  - `validate_record(record, validators)` and `validate_record(record, validators, context)` compose field validators over one map record (**Experimental**): each validator callable in `validators` receives the original `record` and must return an Outcome; `some(...)` field results contribute present values to `clean_record`; `none(...)` means successful absence; `err(...)` results are aggregated into field-level diagnostics; success returns `some(clean_record, context?)`; failure returns `err(quote(record_validation_failed), context_with_diagnostics)`; non-map record, non-map validators, non-callable validators, and non-Outcome validator returns are runtime misuse errors
  - `collect_validated(results)` is a host-backed terminal builtin (Experimental, issue #383) that accepts a Seq-compatible source of Outcome items and returns `{clean: [...], diagnostics: [...]}`; `some(value)` contributes `value` to clean; `none(...)` and `err(...)` produce structured diagnostics with `index`, `kind` (`quote(skipped)` or `quote(error)`), `reason`, and `context`; does not create Sheets itself or change Outcome semantics — pass `clean` to `collect_sheet` for an explicit Sheet conversion
  - `collect_sheet(records)` is a host-backed terminal builtin (Experimental, issue #395) that accepts a Seq-compatible source (list or Flow) of plain map records and returns an immutable Sheet; empty input returns the same zero-shape Sheet as `sheet([])`; the first record's key order becomes column order and every later record must have exactly that key set (order-independent) or it fails with a `TypeError` naming the row and column; values are stored as ordinary cells with no coercion and Outcome values are not unwrapped — a bare `some`/`none`/`err` item is rejected as a non-map record
  - `render_csv(sheet)` is a host-backed pure builtin (Experimental, issue #396) that returns deterministic CSV text without writing it; headers follow Sheet column order, rows follow Sheet row order, CSV-sensitive fields are quoted with doubled quotes, and non-zero-column output ends in `\n`; supported fields are string, symbol, integer, float, boolean, and nil, while composite and non-nil Outcome fields fail with a located `TypeError`; compose with `write(stdout, render_csv(report))` when output is required
  - `row_get(row, column_name)` is a host-backed pure builtin (Experimental, issue #363) that returns the value paired with `column_name` in a row — a list of `[name, value]` pairs, the same shape `rows(sheet)` returns and `where`/`derive` row functions already receive; it does not take a Sheet directly; lookup uses the same column-name identity rules as `sheet`/`select`; a non-list row, a malformed row entry, or a missing column each fail with a located/named `TypeError`; introduces no new syntax
  - `validate_each(source, validator)` is a prelude-backed validation helper (Experimental, issue #392, issue #415) that applies a callable validator to each item of a list or Flow source and returns one Outcome per item; list input returns a list of Outcome values; Flow input returns a lazy Flow of Outcome values; `validator` must be callable and must return an Outcome (`some(...)`, `none(...)`, or `err(...)`); non-Outcome results raise `TypeError("validate_each expected validator to return an Outcome, received <type> at index <n>")`; for Flow input, non-Outcome validator failures occur when the returned Flow is consumed; output order matches source order; does not aggregate — compose with `collect_validated` for aggregation; `validate_each/3` and Sheet behavior are not implemented in this phase
...
- `json_schema(json_represented_schema)` (**Experimental**) compiles the closed structural subset `type`/`properties`/`required`/`items`/boolean `additionalProperties` into an ordinary callable Outcome Template. Unsupported keywords and malformed schemas return normalized `err` Outcomes rather than being ignored.
- `examples/r9_composed_json_template_pipeline.genia` is the directly tested E9-7 integration case: represented JSON records are matched through a JSON Schema-derived `Person` Template and aggregated with existing Outcome validation helpers; it adds no new semantics.
- The portable domain uses safe integers through ±9007199254740991, exact Decimal fraction/exponent values satisfying `stable_json_decimal` (R23 E23-3/E23-4, issue #915/#921 -- never a host binary64 float; NaN/infinity remain invalid), Unicode scalar strings, string object names, and at most 128 nested containers. It adds no JSON-specific runtime value class or Core IR node.
- `json_parse`, `json_stringify`, `json_pretty`, and `parse_jsonl_record` are not aliases for the representation-aware boundary and keep their own `none(...)` failure shape. R23 E23-5 (issue #923) gave them the same lexical Decimal decode as the strict boundary (a fraction/exponent token always decodes to an exact Decimal, never a host float) plus Decimal/terminating Rational/finite Float64 encode support, deliberately without the `stable_json_decimal` stability gate -- legacy JSON tolerance still never rejects a syntactically valid number.
