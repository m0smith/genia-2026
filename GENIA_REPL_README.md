...
- builtins:
  ...
  - `validate_each(source, validator)` is a prelude-backed validation helper (Experimental, issue #392) that applies a callable validator to each item of a list source and returns one Outcome per item; `validator` must be callable and must return an Outcome (`some(...)`, `none(...)`, or `err(...)`); non-Outcome results are runtime errors; output order matches source order; does not aggregate — compose with `collect_validated` for aggregation; `validate_each/3`, Flow input, and Sheet behavior are not implemented in this phase
